...
const clienteController = {
  ...
  incluiRegistro: async (req, res) => {
    try {
      ...
      if (cpfExiste) {
        return res.status(409).json({ message: "CPF já cadastrado na base de dados" });
      }

      const resultado = await clienteModel.insert(nome, cpf);
      if (resultado.insertId === 0) {throw new Error("Erro ao incluir o cliente");
      }
  ...
  alteraCliente: async (req, res) => {
    try {
      ...
      if (cpf && cpf !== clienteAtual[0].cpf_cliente) {
        const clientes = await clienteModel.selelectAll();
        const cpfDuplicado = clientes.find((c) => c.cpf_cliente == cpf && c.id_cliente !== idCliente);
        if (cpfDuplicado) {
          return res.status(409).json({ message: "O novo CPF informado já está cadastrado" });
        }
      }
  ...
  deletaCliente: async (req, res) => {
    try {
      ...
      if (clienteSelecionado.length === 0) {
        return res.status(404).json({ message: "Cliente não localizado na base de dados" });
      }
    ...
    } catch (error) {console.error(error); res.status(500).json({ message: "Erro interno do servidor", errorMessage: error.message });
    }
  },
};
